...
function CustomerReg()
{
    ...
    const[cuserpass,setUserPass]=useState();
    ...
    const handleStIdSelect=(evt)=>{
        ...
        axios.get("http://localhost:9211/city/showcitybystate/"+evt.target.value).
        then((res)=>{
            setCtList(res.data);
        }).catch((err)=>{
            alert(err);
    ...
    const handleRegisterButton=async()=>{
        ...
        const response=await fetch('http://localhost:9211/customer/savecustomerimage',{
            method:'POST',
            body:formData,
        })
        if(response){
       ...
       axios.post("http://localhost:9211/customer/register/",obj).then((res)=>{
        alert(res.data);
        if(res.data=="Registration Successfully")
        {
            axios.post("http://localhost:9211/email/sendemails/"+cemail).then((res)=>{
                alert(res.data);
            }).catch((err)=>{
                alert(err);
            })
    ...
    return(
        <div className="customer-container">
            <center>
                <p className="form-title">Customer Registration Form</p>
                <table className="customer-table">
                    ...
                    <tr>
                        <td>User Id</td>
                        <td>
                            <input type="text" onChange={handleUserIdText} className="form-control"/>
                        </td>
                    </tr>
                    <tr>
                        <td>Password</td>
                        <td>
                            <input type="password" onChange={handleUserPassText} className="form-control"/>
                        </td>
                    </tr>
                    <tr>
                        <td>Customer Name</td>
                    ...
                    <tr>
                        <td>Select Photo</td>
                        <td>
                            <input type="file" onChange={handleFileChange} name="files" className="form-control"/>
                            <img src={image.preview} className="preview-image" width='100' height='100' alt="Preview"/>
                        </td>
                    </tr>
                    <tr>
